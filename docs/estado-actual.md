...
| Transporte (TMS) | `/api/v1/organizations/:orgId/transporte/viajes`, `/transportistas`, `/vehiculos`, `/conductores`, `/rutas` |
| Entrega (POD) | `/api/v1/organizations/:orgId/entregas/pendientes`, `/entregas/viajes/:viajeId`, `/entregas/viajes/:viajeId/confirmar` |
...
- Recepción v1 **está**. Diseño: [recepcion.md](recepcion.md). Ubicaciones, putaway y reubicación entre zonas con confirmación de código **están** (v1: sin QR de cámara ni distancia/FEFO en el putaway). Reserva v1 **está**. Diseño: [reserva.md](reserva.md). Picking, control y consolidación v1 **están**. Palletización outbound (`PAL-DSP`), control de carga y despacho v2 **están**. Transporte TMS v1 y entrega/POD v1 **están** (sin GPS ni recepción de devolución completa). Diseño: [control-consolidacion.md](control-consolidacion.md), [palletizacion-despacho.md](palletizacion-despacho.md), [transporte-entrega.md](transporte-entrega.md). Falta: secuencia LIFO de carga, variantes (talla/gramaje), GPS, retorno al WMS.
